...
const CountryPerformanceChart = ({ data }: any) => {
  console.log("CountyCHart Data",data)
...
export default function FinancialDashboardPage() {
    ...
    const loadData = async () => {
      ...
      try {
        const res = await axios.get(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/managerfinancedashboard/${userData?.countryId}`,
  ...
  const expenseBreakdown =
    apiData?.topExpensesCategory?.map((c: any) => ({
      category: c.category,
      amount: c.total_amount,
      color: "bg-red",
    })) || [];

  const leadingManagerData = apiData?.topPerformerCountry?.find(
    (m: any) => m.manager_name
  ) || {
    name: "No Manager",
    title: "",
    performanceMetric: 0,
  ...
  const countryPerformanceData =
    apiData?.topPerformerCountry?.map((c: any) => ({
  ...
  return (
    <div className="min-h-screen ml-26 bg-gray-50 p-4 md:p-8">
            ...
            {expenseBreakdown.map((item: any, index: number) => (
              <div key={index} className="space-y-1">
                ...
                <Progress
                  ...
                  className="h-2 bg-red-200 [&>div]:bg-red-500"
                />
              </div>
            ))}
          </CardContent>
      ...
      {/* --- Dialogs --- */}
      <AddExpenseDialog isOpen={isExpenseOpen} onClose={() => setIsExpenseOpen(false)}
       onSave={(data) => {
          console.log("Donation saved", data);
          setIsDonationOpen(false);
          // optionally call your API to save
        }} />
      <AddDonationDialog
