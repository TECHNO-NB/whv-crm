...
import { DollarSign, BarChart3, TrendingUp, Handshake, Calendar, ArrowUpRight, ArrowDownRight, Goal, List, PiggyBank, Briefcase, User, PlusCircle, Globe, Wallet, TrendingDown, Award } from 'lucide-react';
...
const GENERAL_INCOME_SOURCES = incomeSources.filter(s => s.source !== 'Individual Donations').map(s => s.source);
...
interface CountrySelectProps { value: string; onChange: (value: string) => void; label?: string; includeAll?: boolean; }
const CountrySelect: React.FC<CountrySelectProps> = ({ value, onChange, label, includeAll = false }) => (
    <div className="space-y-2">
        ...
        <Select value={value} onValueChange={onChange}>
            ...
            <SelectContent>
                {COUNTRIES.filter(c => includeAll || c.value !== 'all').map(country => (
...
interface YearSelectProps { value: number; onChange: (value: number) => void; label?: string; }
...
interface DialogProps { isOpen: boolean; onClose: () => void; onSave: (data: any) => void; }
const AddExpenseDialog: React.FC<DialogProps> = ({ isOpen, onClose, onSave }) => {
    const [amount, setAmount] = useState('');
    const [category, setCategory] = useState<string>('');
    const [country, setCountry] = useState<string>('');
    const [description, setDescription] = useState('');
    const handleSave = () => { /* ... implementation ... */ onSave({ type: 'Expense', amount: parseFloat(amount), category, description, country, date: new Date().toISOString() }); setAmount(''); setCategory(''); setCountry(''); setDescription(''); onClose(); };
    return (<Dialog open={isOpen} onOpenChange={onClose}><DialogContent className="sm:max-w-[425px}"><DialogHeader><DialogTitle className="flex items-center"><List className="w-5 h-5 mr-2 text-red-500"/> Add New Expense</DialogTitle><DialogDescription>Record a new outgoing transaction. Required fields are marked.</DialogDescription></DialogHeader><div className="grid gap-4 py-4"><div className="space-y-2"><Label htmlFor="expense-amount">Amount ($)</Label><Input id="expense-amount" placeholder="e.g., 5000.00" type="number" value={amount} onChange={(e) => setAmount(e.target.value)} required /></div><div className="space-y-2"><Label>Category</Label><Select value={category} onValueChange={setCategory}><SelectTrigger><SelectValue placeholder="Select expense category" /></SelectTrigger><SelectContent>{EXPENSE_CATEGORIES.map(cat => (<SelectItem key={cat} value={cat}>{cat}</SelectItem>))}</SelectContent></Select></div><CountrySelect value={country} onChange={setCountry} label="Country" /><div className="space-y-2"><Label htmlFor="expense-description">Description (Optional)</Label><Textarea id="expense-description" placeholder="Short memo for this expense" value={description} onChange={(e) => setDescription(e.target.value)} /></div></div><DialogFooter><Button variant="outline" onClick={onClose}>Cancel</Button><Button onClick={handleSave} disabled={!amount || !category || !country} className="bg-red-500 hover:bg-red-600">Add Expense</Button></DialogFooter></DialogContent></Dialog>);
};
const AddDonationDialog: React.FC<DialogProps> = ({ isOpen, onClose, onSave }) => {
    const [amount, setAmount] = useState('');
    const [donorName, setDonorName] = useState('');
    const [country, setCountry] = useState<string>('');
    const handleSave = () => { /* ... implementation ... */ onSave({ type: 'Donation', amount: parseFloat(amount), donorName, country, date: new Date().toISOString() }); setAmount(''); setDonorName(''); setCountry(''); onClose(); };
    return (<Dialog open={isOpen} onOpenChange={onClose}><DialogContent className="sm:max-w-[425px}"><DialogHeader><DialogTitle className="flex items-center"><PiggyBank className="w-5 h-5 mr-2 text-green-500"/> Record Donation</DialogTitle><DialogDescription>Input details for a new individual donation received.</DialogDescription></DialogHeader><div className="grid gap-4 py-4"><div className="space-y-2"><Label htmlFor="donation-amount">Amount ($)</Label><Input id="donation-amount" placeholder="e.g., 50.00" type="number" value={amount} onChange={(e) => setAmount(e.target.value)} required /></div><div className="space-y-2"><Label htmlFor="donor-name">Donor Name / ID</Label><Input id="donor-name" placeholder="John Doe or Donor #1234" value={donorName} onChange={(e) => setDonorName(e.target.value)} required /></div><CountrySelect value={country} onChange={setCountry} label="Country of Donor" /></div><DialogFooter><Button variant="outline" onClick={onClose}>Cancel</Button><Button onClick={handleSave} disabled={!amount || !donorName || !country} className="bg-green-500 hover:bg-green-600">Record Donation</Button></DialogFooter></DialogContent></Dialog>);
};
const AddIncomeDialog: React.FC<DialogProps> = ({ isOpen, onClose, onSave }) => {
    const [amount, setAmount] = useState('');
    const [source, setSource] = useState<string>('');
    const [country, setCountry] = useState<string>('');
    const handleSave = () => { /* ... implementation ... */ onSave({ type: 'General Income', amount: parseFloat(amount), source, country, date: new Date().toISOString() }); setAmount(''); setSource(''); setCountry(''); onClose(); };
    return (<Dialog open={isOpen} onOpenChange={onClose}><DialogContent className="sm:max-w-[425px}"><DialogHeader><DialogTitle className="flex items-center"><Briefcase className="w-5 h-5 mr-2 text-blue-500"/> Add General Income</DialogTitle><DialogDescription>Record income from grants, partnerships, or fees.</DialogDescription></DialogHeader><div className="grid gap-4 py-4"><div className="space-y-2"><Label htmlFor="income-amount">Amount ($)</Label><Input id="income-amount" placeholder="e.g., 25000.00" type="number" value={amount} onChange={(e) => setAmount(e.target.value)} required /></div><div className="space-y-2"><Label>Income Source</Label><Select value={source} onValueChange={setSource}><SelectTrigger><SelectValue placeholder="Select income source" /></SelectTrigger><SelectContent>{GENERAL_INCOME_SOURCES.map(src => (<SelectItem key={src} value={src}>{src}</SelectItem>))}</SelectContent></Select></div><CountrySelect value={country} onChange={setCountry} label="Country of Origin" /></div><DialogFooter><Button variant="outline" onClick={onClose}>Cancel</Button><Button onClick={handleSave} disabled={!amount || !source || !country} className="bg-blue-500 hover:bg-blue-600">Add Income</Button></DialogFooter></DialogContent></Dialog>);
};
const AddBudgetDialog: React.FC<{ isOpen: boolean; onClose: () => void; onSave: (data: any) => void; availableYears: number[] }> = ({ isOpen, onClose, onSave, availableYears }) => {
    ...
    const handleSave = () => { /* ... implementation ... */ onSave({ year, country, incomeBudget: parseFloat(incomeBudget), expenseBudget: parseFloat(expenseBudget) }); setCountry(''); setIncomeBudget(''); setExpenseBudget(''); onClose(); };
    return (<Dialog open={isOpen} onOpenChange={onClose}><DialogContent className="sm:max-w-[425px}"><DialogHeader><DialogTitle className="flex items-center"><Wallet className="w-5 h-5 mr-2 text-orange-500"/> Set Annual Budget</DialogTitle><DialogDescription>Define income and expense budgets for a specific country and year.</DialogDescription></DialogHeader><div className="grid gap-4 py-4"><YearSelect value={year} onChange={setYear} label="Financial Year" /><CountrySelect value={country} onChange={setCountry} label="Country Scope" includeAll={true} /><div className="space-y-2"><Label htmlFor="income-budget">Income Budget ($)</Label><Input id="income-budget" placeholder="e.g., 1000000" type="number" value={incomeBudget} onChange={(e) => setIncomeBudget(e.target.value)} required /></div><div className="space-y-2"><Label htmlFor="expense-budget">Expense Budget ($)</Label><Input id="expense-budget" placeholder="e.g., 800000" type="number" value={expenseBudget} onChange={(e) => setExpenseBudget(e.target.value)} required /></div></div><DialogFooter><Button variant="outline" onClick={onClose}>Cancel</Button><Button onClick={handleSave} disabled={!year || !country || !incomeBudget || !expenseBudget} className="bg-orange-500 hover:bg-orange-600">Set Budget</Button></DialogFooter></DialogContent></Dialog>);
};
...
const LeadingManagerProfile: React.FC<{ manager: ManagerData }> = ({ manager }) => {
    return (
            ...
            <CardContent className="flex items-center space-x-4 pt-4">
                ...
                <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                        **{formatCurrency(manager.performanceMetric)}** ({manager.metricLabel})
                    </p>
                </div>
            </CardContent>
        </Card>
    ...
    return (
        <Card className="shadow-lg lg:col-span-3">
            <CardHeader className="pt-4 px-6">
                ...
                <p className="text-sm text-gray-500">Comparing top 3 and bottom 3 countries by Net Balance (Income - Expenses).</p>
            </CardHeader>
...
export default function FinancialDashboardPage() {
    ...
    const handleAddExpense = (data: any) => { console.log("New Expense Added:", data); };
    const handleAddDonation = (data: any) => { console.log("New Donation Recorded:", data); };
    const handleAddIncome = (data: any) => { console.log("New General Income Added:", data); };
    const handleAddBudget = (data: any) => { console.log("New Budget Set:", data); setYearlyBudgets(prev => [...prev, data]); };

    // --- Data Calculations/Filtering (SIMULATED) ---
    const currentBudget = useMemo(() => {
        const countryBudget = yearlyBudgets.find(b => b.year === selectedYear && b.country === selectedCountry);
        if (countryBudget) return countryBudget;
        const allCountryBudget = yearlyBudgets.find(b => b.year === selectedYear && b.country === 'all');
        return allCountryBudget || { incomeBudget: 0, expenseBudget: 0, year: selectedYear, country: selectedCountry };
    ...
    const budgetIncomeProgress = Math.min(100, Math.round((financialData.totalIncome / currentBudget.incomeBudget) * 100));
    const budgetExpenseProgress = Math.min(100, Math.round((financialData.totalExpenses / currentBudget.expenseBudget) * 100));
        ...
        <div className="min-h-screen ml-26 bg-gray-50 p-4 md:p-8">
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center space-y-3 sm:space-y-0 sm:space-x-4 w-full xl:w-auto">
                    ...
                    <div className="flex space-x-4">
                        ...
                        <div className="min-w-[200px]">
                            <CountrySelect value={selectedCountry} onChange={setSelectedCountry} label="Filter by Country" includeAll={true} />
                        </div>
                    </div>
                    
                    {/* Add Transaction Buttons */}
                    <div className="flex space-x-2 justify-end">
                        <Button onClick={() => setIsBudgetOpen(true)} className="bg-orange-500 hover:bg-orange-600 text-white shadow-md flex-shrink-0">
                        ...
                        <Button onClick={() => setIsIncomeOpen(true)} className="bg-blue-500 hover:bg-blue-600 text-white shadow-md flex-shrink-0 hidden md:inline-flex">
                         ...
                         <Select onValueChange={(val) => {
                             ...
                             <SelectTrigger className="md:hidden w-fit flex-shrink-0">
                                 <PlusCircle className="w-4 h-4 mr-2" /> <SelectValue placeholder="Add..." />
                             </SelectTrigger>
                             <SelectContent>
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 mb-6">
                ...
                <Card className="shadow-lg hover:shadow-xl transition-shadow border-t-4 border-emerald-500">
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <CardTitle className="text-sm font-medium text-gray-500">Income Budget Progress ({selectedYear})</CardTitle>
                        <Wallet className="w-4 h-4 text-emerald-500" />
                    </CardHeader>
                    <CardContent>
                        <div className="text-3xl font-bold text-gray-800">{budgetIncomeProgress}%</div>
                        <p className="text-xs text-muted-foreground mt-1">
                            {formatCurrency(financialData.totalIncome)} / {formatCurrency(currentBudget.incomeBudget)}
                        </p>
                        <Progress value={budgetIncomeProgress} className="h-2 mt-2 bg-emerald-200 [&>div]:bg-emerald-500" />
                    </CardContent>
                </Card>
                        ...
                        {expenseBreakdown.map((item, index) => (
                            <div key={index} className="space-y-1">
                                ...
                                <Progress
                                    ...
                                    className={`h-2 ${item.color.replace('bg-', 'bg-')}-200 [&>div]:bg-red-500`}
                                />
                            </div>
                        ))}
                    </CardContent>
            ...
            <div className="grid gap-6 lg:grid-cols-2 mb-8">
                ...
                <Card className="shadow-lg">
                    <CardHeader className="pt-4 px-6">
                        ...
                        <p className="text-sm text-gray-500">Year: {selectedYear} | Country: {COUNTRIES.find(c => c.value === selectedCountry)?.label}</p>
                    </CardHeader>
                    <CardContent className="h-80 pt-6">
                        <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={monthlyFinancials}>
                                ...
                                <Tooltip content={<CustomTooltip />} />
                                <Legend wrapperStyle={{ paddingTop: '10px' }} />
                                <Line type="monotone" dataKey="income" stroke="#10B981" activeDot={{ r: 8 }} name="Income" strokeWidth={3} />
                                <Line type="monotone" dataKey="expenses" stroke="#EF4444" activeDot={{ r: 8 }} name="Expenses" strokeWidth={3} />
                            </LineChart>
                        </ResponsiveContainer>
                    </CardContent>
                </Card>
            ...
            {/* --- MODALS --- */}
            <AddExpenseDialog isOpen={isExpenseOpen} onClose={() => setIsExpenseOpen(false)} onSave={handleAddExpense} />
            <AddDonationDialog isOpen={isDonationOpen} onClose={() => setIsDonationOpen(false)} onSave={handleAddDonation} />
            <AddIncomeDialog isOpen={isIncomeOpen} onClose={() => setIsIncomeOpen(false)} onSave={handleAddIncome} />
            <AddBudgetDialog isOpen={isBudgetOpen} onClose={() => setIsBudgetOpen(false)} onSave={handleAddBudget} availableYears={YEARS} />
