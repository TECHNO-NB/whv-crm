...
const uid = (prefix = "") =>
  `${prefix}${Math.random().toString(36).slice(2, 9)}${Date.now().toString(
    36
  )}`;
...
export default function Page() {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      const normalized = {
        ...created,
        attachments: Array.isArray(created?.attachments)
          ? created.attachments
          : [],
      };
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 sm:p-10 font-sans ml-28">
      ...
      <p className="text-sm text-gray-500 mb-6">
        Tickets are loaded from the server API.
      </p>
      ...
      <div className="flex flex-wrap gap-4 mb-8 p-4 bg-white rounded-xl shadow-lg border border-gray-100">
        ...
        <div className="ml-auto flex items-end">
          <div className="text-sm p-2 rounded-lg bg-blue-100 text-blue-800 font-semibold shadow-inner">
            Total Tickets: {(tickets || []).length} (Showing:{" "}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {filteredTickets.map((ticket) => (
          <div
            key={ticket.id}
            className="bg-white p-5 rounded-xl shadow-lg border border-gray-200 hover:shadow-xl transition-all duration-300 space-y-3"
          >
            {/* Header */}
            <div className="flex justify-between items-start flex-wrap">
              <div className="flex-1 min-w-0">
                <p className="text-xs font-mono text-gray-500 mb-1 truncate">
                  ID: {ticket.id}
                </p>
              ...
              <div
                className={`text-xs font-semibold uppercase px-3 py-1 rounded-full border ${statusStyles(
                  ticket.status
                )} flex-shrink-0 mt-2 md:mt-0`}
              >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 border-t pt-3 mt-3">
              ...
              <div>
                ...
                <Badge className={`mt-1 ${priorityBadge(ticket.priority)}`}>
                  {ticket.priority}
                </Badge>
              </div>
                ...
                {(ticket.attachments || []).length > 0 ? (
                  <div className="flex flex-wrap gap-2 mt-1">
                    {(ticket.attachments || []).map((att) => (
                      <div
                        key={att}
                        className="relative bg-gray-50 border rounded-md p-1 w-32 h-20 flex flex-col items-center justify-center overflow-hidden"
                      >
                        {att ? (
                          <img
                            src={att}
                        ...
                        <div className="absolute top-1 right-1 flex gap-1">
                          <button
                            title="Remove"
                            onClick={() =>
                              removeAttachmentFromTicket(ticket.id, att.id)
                            }
                          ...
                          {att.url && (
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="bg-white/80 p-1 rounded-full hover:bg-gray-100"
                            >
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl mt-10">
            <div className="flex items-center justify-between p-5 border-b">
              <h3 className="text-xl font-semibold flex items-center gap-2">
                <TicketIcon className="w-5 h-5 text-blue-600" /> Create New
                Ticket
              </h3>
              <button
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="p-4 border border-dashed rounded-md bg-gray-50">
                ...
                <p className="text-xs text-gray-500 mt-2">
                  You can add multiple files. Image previews will be shown.
                  Remove any before submitting if not needed.
                </p>
                ...
                {attachmentsBeforeUpload.length > 0 && (
                  <div className="mt-3 grid grid-cols-3 sm:grid-cols-4 gap-3 max-h-48 overflow-y-auto">
                    {attachmentsBeforeUpload.map((a) => (
                      <div
                        key={a.id}
                        className="relative bg-white border rounded-md p-1 w-full h-24 flex flex-col items-center justify-center overflow-hidden"
                      >
                        {a.previewUrl ? (
                          <img
                            src={a.previewUrl}
                          ...
                          <div className="flex flex-col items-center justify-center text-xs text-gray-600 p-2">
                            <Paperclip className="w-4 h-4 mb-1" />
                            <span className="truncate w-full text-[12px]">
                              {a.name}
                            </span>
                          </div>
                        )}
